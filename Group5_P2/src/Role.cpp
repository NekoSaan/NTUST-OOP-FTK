...
// Intent: Construct a Role object.
// Pre: None
// Post: Initializes a Role with default values for weapon, armor, and accessory
...
// Intent: Move the Role to its last position.
// Pre: None
// Post: Role's position is set to the last position.
void Role::moveTolastPos() {
	setPos(lastPos.first, lastPos.second);
}

// Intent: Increase the Role's health points.
// Pre: healHp is the amount of health points to be added.
// Post: Role's health points are increased, capped at its maximum vitality.
void Role::gainHealth(int healHp) {
	hp = min(hp + healHp, vitality);
}

// Intent: Increase the Role's focus points.
// Pre: restoreN is the amount of focus points to be added.
// Post: Role's focus points are increased, capped at its maximum focus.
void Role::gainFocus(int restoreN) {
	focus = min(focus + restoreN, maxFocus);
}

// Intent: Set the movement points of the Role based on its speed and passive skills.
// Pre: None
// Post: Role's movement points are calculated and updated according to its speed and passive skills.
void Role::setMovementPoint() {
	int maxMovementPoint = speed / 10;

	// Calculate movement points based on whether the Role has the "Run" passive skill.
...
// Intent: Get the current movement points of the Role.
// Pre: None
// Post: Returns the current movement points of the Role.
int Role::getMovementPoint() {
	return movementPoint;
}

// Intent: For virutal override
// Pre: Nothing
// Post: Nothing
...
// Intent: Select a target from the enemy list.
// Pre: role and enemy vectors must contain valid Entity pointers.
// Post: Returns the index of the selected target from the enemy vector.
...
// Intent: Allow the Role to use focus points during combat.
// Pre: MaxFocus is the maximum amount of focus points the Role can use. role and enemy vectors must contain valid Entity pointers.
// Post: Returns the amount of focus points used by the Role and updates its focus points accordingly.
...
// Intent: Allow the Role to attempt fleeing from combat.
// Pre: role and enemy vectors must contain valid Entity pointers.
// Post: Returns 1 if the Role successfully flees, 0 otherwise. Focus points are consumed during the attempt.
